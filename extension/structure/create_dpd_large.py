...
if __name__ == '__main__':
    ...
    fp = "/home/centos/ztz/dpd_hybrid_large/dpd-small/"
    os.makedirs(fp, exist_ok=True)
    LmpParser.create_data_file(atom3d, fp + "data.%dblk_%d" % (blocks, chains), q=False, improper=False, dihedral=False)

    atom3d = LmpParser.load_data_file(fp + "data.%dblk_%d" % (blocks, chains), _type="dpd_fg")
    atom3d.convert_to_fg(h=True)
    LmpParser.create_data_file(atom3d, fp + "data.%dblk_%d" % (blocks, chains), q=False, improper=True, dihedral=True)
