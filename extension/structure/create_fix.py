...
if __name__ == '__main__':
    ...
    atom3d = csc.Atom3D()
    atom3d.create_from_pattern_fix(pattern, chain_num, param=param1, density=1., center=10)
    cg_fp = "/home/centos/work/fix/cg_%dblk_%dchain/init/" % (blk_num, chain_num)
    if not os.path.exists(cg_fp):
        os.makedirs(cg_fp)
    LmpParser.create_data_file(atom3d, cg_fp + "strain.data", q=False, improper=False)

    atom3d.cg_to_cgu()
    cgu_fp = "/home/centos/work/fix/cgu_%dblk_%dchain/init/" % (blk_num, chain_num)
